...
import { StateSelector } from "../redux/appReducer";
...
const BookModal: React.FC<ModalProps> = ({ onClose }) => {
  const { currentBook } = useSelector((state: StateSelector) => ({
  ...
  return (
    <div className="book-modal">
      <div className="book-modal__main">
        <div className="book-modal__cover">
          <img
            src={
              currentBook.img
                ? `https://covers.openlibrary.org/b/id/${currentBook.img}-L.jpg`
                : undefinedImg
            }
            alt="Book cover"
          />
        ...
        <div className="book-modal__info">
          <div className="book-modal__header">
            <h3 className="book-modal__title">{currentBook.title}</h3>
            <h5 className="book-modal__author">{currentBook.author}</h5>
          </div>
          <div className="book-modal__content">
            <div className="book-modal__item">
              Дата публикации: <span>{currentBook.firstPublished}</span>
            </div>
            <div className="book-modal__item">
              Издатель: <span>{currentBook.edition}</span>
            </div>
            <div className="book-modal__item">
              ISBN: <span>{currentBook.ISBN}</span>
            </div>
          </div>
        </div>
        <i onClick={onClose} className="book-modal__close">
          <svg
            width="21"
            height="21"
            viewBox="0 0 21 21"
      ...
      <div className="book-modal__overlay"></div>
    </div>
  );
};
